...
def simulate_case2_property_strategy(monthly_stock_investment_before_mortgage,
    ...
    for month in range(1, simulation_months + 1):
        if not property_owned:
            ...
            # No stock investing before property purchase
            stocks = stocks * (1 + monthly_stock_return)
            ...
            if mortgage_balance > 0:
                ...
                # Start investing once mortgage begins
                monthly_investment = monthly_stock_investment_before_mortgage
            else:
                # Mortgage finished
                # Invest mortgage cashflow too
...
def plot_comparison(month_limit, comparison):
    ...
    # Case 1: Blue
    plt.plot(data["Month"], data["Case1_StocksOnly"], linewidth=4, color=CASE1_COLOR,
             label="Case 1: Stocks Only (13% return, 5k/month)")
    
    # Case 2: Orange
    plt.plot(data["Month"], data["Case2_PropertyStrategy"], linewidth=4, color=CASE2_COLOR,
    ...
    for i in range(1, len(data)):
        if data["Difference"].iloc[i-1] * data["Difference"].iloc[i] < 0:
            # Sign changed between month i-1 and i
            cross_month = i   # or i-0.5 if you prefer midpoint
            sign_changes.append(cross_month)
    for cross_month in sign_changes:
        if cross_month > 1:
            plt.axvline(cross_month, linestyle="--", linewidth=2, alpha=0.7, color='gray',
                        label='_nolegend_')
        # Orange shading when Property wins
        plt.fill_between(
            data["Month"],
            data["Case1_StocksOnly"],
            data["Case2_PropertyStrategy"],
        ...
        # Blue shading when Stocks win
        plt.fill_between(
            data["Month"],
            data["Case1_StocksOnly"],
            data["Case2_PropertyStrategy"],
            where=(data["Case1_StocksOnly"] > data["Case2_PropertyStrategy"]),
            alpha=0.15,
            color=CASE1_COLOR,
            interpolate=True,
            label='_nolegend_'
    ...
    plt.scatter(max_diff_row["Month"], max_diff_row["Case2_PropertyStrategy"],
                s=120, color=MAX_POINT_COLOR, zorder=5, label='Maximum Advantage Point')
...
# ========================
plt.figure(figsize=(16, 8))
plt.plot(comparison["Month"], comparison["Difference"], linewidth=3, color='dimgrey')
